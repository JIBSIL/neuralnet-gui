<template>
  <div class="bg-white p-6 rounded-lg flex-1">
    ...
    <p class="text-gray-500">This editor allows adding data manually. For larger datasets, try the Data Selection utility.
    </p>
    <div id="jsonEditor" class="mt-4" v-if="!showJson">
      <div v-for="(row, index) in jsonEditorRows" :key="index" class="grid grid-cols-4 gap-2 mb-2">
        ...
        <select v-model="row.type" @change="openModal(index)" class="border p-2 rounded">
          ...
          <option :disabled="!lstm">String (LSTM-only)</option>
          <option>Number</option>
          <option :disabled="lstm">Boolean</option>
          <option>Array</option>
          <option :disabled="lstm">Object</option>
        </select>
        <input type="text" v-if="row.type === 'String'" v-model="row.value" placeholder="Enter value"
          class="border p-2 rounded">
        <input type="number" v-if="row.type === 'Number'" v-model.number="row.value" placeholder="Enter value"
          class="border p-2 rounded">
        <select v-if="row.type === 'Boolean'" v-model="row.value" class="border p-2 rounded">
          <option>true</option>
          <option>false</option>
        </select>
        <input type="text" v-if="row.type === 'Array' || row.type === 'Object'" v-model="row.output"
          placeholder="JSON output" class="border p-2 rounded" readonly>
        <div class="flex items-center">
          <button class="bg-red-500 text-white p-2 rounded ml-2 inline-flex items-center" @click="removeRow(index)">
            <svg class="w-4 h-4 mr-1" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M10 1C5.03 1 1 5.03 1 10C1 14.97 5.03 19 10 19C14.97 19 19 14.97 19 10C19 5.03 14.97 1 10 1ZM13 13H7V7H13V13Z"
                fill="currentColor" />
            </svg>
            Remove
          </button>
    ...
    <div class="flex" v-if="!showJson">
      ...
      <button class="bg-green-500 text-white p-2 rounded inline-flex items-center ml-2" @click="toggleJsonEditor">Save
        Dataset</button>
    </div>

    <!-- Array Modal -->
    <div v-if="showArrayModal" class="fixed z-10 inset-0 overflow-y-auto flex items-center justify-center"
      aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div class="fixed inset-0 bg-black opacity-50"></div> <!-- Overlay -->
      <div
        class="relative inline-block align-middle bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all my-8 align-middle max-w-lg w-full">
        <div class="p-6">
          ...
          <div v-for="(item, index) in arrayModalData" :key="index" class="mt-2">
            <input type="number" :value="item" @input="updateArrayElement($event, index)" placeholder="Enter element"
              class="border p-2 rounded">
          </div>
    ...
    <!-- Object Modal -->
    <div v-if="showObjectModal" class="fixed z-10 inset-0 overflow-y-auto flex items-center justify-center"
      aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div class="fixed inset-0 bg-black opacity-50"></div> <!-- Overlay -->
      <div
        class="relative inline-block align-middle bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all my-8 align-middle max-w-lg w-full">
        <div class="p-6">
          ...
          <div v-for="(item, index) in objectModalData" :key="index" class="mt-2 grid grid-cols-2 gap-2">
            <input type="text" :value="item.key" @input="updateObjectKey($event, index)" placeholder="Enter key"
              class="border p-2 rounded">
            <input type="number" :value="item.value" @input="updateObjectValue($event, index)" placeholder="Enter value"
              class="border p-2 rounded">
          </div>
...
export default {
  ...
  mounted() {
    this.emitter.on('model-changed', (isLSTM) => {
      this.lstm = isLSTM;
      console.log(`New setting for LSTM: ${isLSTM}`)
    });
  },
  methods: {
    addNewRow() {
    ...
    toggleJsonEditor() {
      ...
      if (this.showJson) {
        ...
        const dataset = this.jsonEditorRows.map((row) => {
        ...
        // let dataset = JSON.parse(this.jsonCode);
        ...
        this.jsonCode = JSON.stringify(dataset, null, 2);
